            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    @if($borrowings->isEmpty())
                        <p class="text-gray-500 text-center py-8">No borrowings found.</p>
                    @else
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            @if($borrowing->status === 'borrowed')
                                                <form action="{{ route('borrowings.return', $borrowing) }}" method="POST" class="inline" onsubmit="return confirm('Mark as returned?')">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="text-green-600 hover:text-green-900">Mark Returned</button>
                                                </form>
                                            @else
                                                <span class="text-gray-400">{{ $borrowing->returned_at->format('M d, Y') }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
